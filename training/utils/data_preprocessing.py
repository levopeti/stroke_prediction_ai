...
from ai_utils.training_utils.clear_measurements import all_key_combination, Key, MeasType
...
def get_3d_arrays_from_df(meas_df: pd.DataFrame) -> Dict[Key, np.ndarray]:
    ...
    for side, limb, meas_type in all_key_combination:
        try:
            result_array = np.concatenate([np.expand_dims(meas_df[str((side.value, limb.value, meas_type.value, "x"))].values, axis=1),
...
def divide_values(meas_1d_arrays: Dict[Key, np.ndarray], div_value: Union[int, float], meas_type: MeasType) -> Dict[Key, np.ndarray]:
    for key, array_1d in meas_1d_arrays.items():
        if key[2] == meas_type:
